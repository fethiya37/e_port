...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  /// Returns {start: GC DateTime, end: GC DateTime, includesPagume: bool}
  Map<String, dynamic> _coverageGC() {
    ...
    if (target!.isWeekly) {
      ...
      final start = gcWeekStartMonday(base); // EC-week aligned Monday (GC)
  ...
  Map<String, num> _totals() {
    ...
    final fee = target!.policy.planFee; // correct fee for plan
    final hasOverdue = _hasOverdue();

    final num interest = hasOverdue ? (target!.interestAccrued) : 0;
    final num overdueBase = hasOverdue ? fee : 0; // cannot skip overdue
  ...
  Future<void> _pay() async {
    ...
    final plateMaybe =
        _searchCtrl.text.contains('-') ? _searchCtrl.text.toUpperCase() : null;
    final cov = _coverageGC(); // {'start': DateTime, 'end': DateTime}
  ...
  Widget build(BuildContext context) {
    ...
    final activeUntilEc =
        target?.activeUntilDate == null ? '—' : ecFromIsoShort(target!.activeUntilDate!);
    ...
    return Scaffold(
      body: Container(
        ...
        child: Column(
          children: [
            // ===== Gradient header (same as RouteAssignments) =====
            Container(
              height: headerH,
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            hintText: 'Enter plate (AA-123456) or phone',
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (step == PaymentStep.details && target != null) ...[
                          // Driver card (white, consistent with Route UI)
                          _DriverCard(
                            name: target!.name,
                            phone: target!.phone,
                                ...
                                Builder(
                                  builder: (_) {
                                    ...
                                    if (isoTo != null) {
                                      ...
                                      final alt = paymentResult!['new_active_until_date']
                                          ?.toString();
...
// ===== Cards (match RouteAssignments look) =====
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: _PaymentScreenState._gradB.withOpacity(0.25)),
                ),
                child: Text(planLabel,
                    style: const TextStyle(fontSize: 11, color: _PaymentScreenState._gradB)),
              ),
            ],
          ),
          const SizedBox(height: 6),
